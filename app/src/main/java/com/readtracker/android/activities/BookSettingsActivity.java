...
import static com.readtracker.android.support.GoogleBooksSearchService.GoogleBookDto;
...
import static com.readtracker.android.support.GoogleBooksSearchService.GoogleCoverImageDto;
 ...
 */
public class BookSettingsActivity extends BookBaseActivity {
    ...
    EditText mTitleEdit;
    @InjectView(R.id.author_edit)
    EditText mAuthorEdit;
    @InjectView(R.id.page_count_edit)
    EditText mPageCountEdit;
    @InjectView(R.id.add_or_save_button)
    Button mSaveButton;
    @InjectView(R.id.track_using_pages)
    CheckBox mTrackUsingPages;
    @InjectView(R.id.book_cover_image)
    ImageButton mCoverImageButton;
    @InjectView(R.id.find_cover_button)
    TextView mFindCoverButton;
    ...
    public void onSearchResultsRetrieved(List<GoogleVolumeDto> result) {
        ...
        if (result != null && result.size() > 0) {
            for (int i = 0; i < result.size(); i++) {
                GoogleBookDto book = result.get(i).getBook();
                if (book.isValid()) {
                    GoogleCoverImageDto cover = result.get(i).getBook().getCoverUrl();
                    if (cover != null && !cover.getThumbnail().isEmpty()) {
                        loadCoverFromURL(cover.getThumbnail());
                        didFindCover = true;
                        break;
                    }
                }
    ...
    private void bindEvents() {
        ...
        mFindCoverButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                final String searchQuery = buildQueryForTitleAndAuthor(title, author);
                if (searchQuery != null) {
                    ...
                    Call<GoogleBooksResponse> call = postApi.getVolumes(searchQuery);

                    call.enqueue(new retrofit2.Callback<GoogleBooksResponse>() {
                        @Override
                        public void onResponse(
                                Call<GoogleBooksResponse> call,
                                Response<GoogleBooksResponse> response) {
                            if (response.isSuccessful()) {
                                onSearchResultsRetrieved(response.body().getItems());
                            } else {
                                Log.d(
                                        TAG,
        ...
        mCoverImageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Toast.makeText(BookSettingsActivity.this, R.string.add_book_long_press_to_delete, Toast.LENGTH_SHORT)
                        .show();
            }
        });
    ...
    public static String buildQueryForTitleAndAuthor(String title, String author) {
        ...
        if(!TextUtils.isEmpty(title)) query.add("intitle:" + title);
        if(!TextUtils.isEmpty(author)) query.add("inauthor:" + author);
        if(query.size() > 0) return TextUtils.join(" ", query);
        return null;
    }
    ...
    static class UpdateBookTask extends BackgroundBookTask {
        ...
        @Override
        protected void onComplete(BookSettingsActivity activity, boolean success) {
