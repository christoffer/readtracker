...
package com.readtracker_beta.thirdparty.widget;
...
import com.readtracker_beta.thirdparty.widget.adapters.WheelViewAdapter;
...
public class WheelView extends View {
  ...
  private static final int DEFAULT_SCROLLING_SPEED = 400; // ms
  ...
  WheelScroller.ScrollingListener scrollingListener = new WheelScroller.ScrollingListener() {
    ...
    public void onJustify() {
      if(Math.abs(scrollingOffset) > WheelScroller.MIN_DELTA_FOR_SCROLLING) {
        scroller.scroll(scrollingOffset, scrollingSpeed);
      }
    }
  };
  ...
  public void setCurrentItem(int index, boolean animated) {
    ...
    if(index != currentItem) {
      if(animated) {
        ...
        scroll(itemsToScroll, scrollingSpeed);
      } else {
        scrollingOffset = 0;
  ...
  private int _getOffset() {
    return (int) (getItemHeight() / 2 * 1.2);
  }

  /**
   * Draws the covering gradient over the view
   *
   * @param canvas the canvas for drawing
   */
